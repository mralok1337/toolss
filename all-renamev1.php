...
// Rename semua file dengan nama baru + ekstensi baru
function renameAllFiles($dir, $newName, $newExt)
{
    $files = scandir($dir);
    foreach ($files as $file) {
        ...
        $path = $dir . '/' . $file;
        if (is_file($path)) {
            $newFile = $dir . '/' . $newName . '.' . ltrim($newExt, '.');
            // Cegah overwrite dengan menambahkan angka jika perlu
            $i = 1;
            while (file_exists($newFile)) {
                $newFile = $dir . '/' . $newName . '_' . $i . '.' . ltrim($newExt, '.');
                $i++;
            }
            rename($path, $newFile);
        }
...
// Rename semua direktori dengan prefix baru
...
if (isset($_POST['action'])) {
    if ($_POST['action'] === 'rename_files') {
        renameAllFiles($dir, $_POST['newname'], $_POST['newext']);
...
<form method="post">
    <h3>🔤 Rename Semua File & Ubah Ekstensi</h3>
